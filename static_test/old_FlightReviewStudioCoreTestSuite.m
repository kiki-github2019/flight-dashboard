classdef old_FlightReviewStudioCoreTestSuite < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function test_T2_Model_RoundTrip(testCase)
            ...
            testCase.verifyEqual( ...
                ...
                2, ...
                'Save ??ProjectModel??session???ㅼ젣濡?異붽??섏? ?딆븯?듬땲??');
            ...
            testCase.verifyEqual( ...
                loaded.sessionCount(), ...
                2, ...
                '濡쒕뱶???꾨줈?앺듃???몄뀡 紐⑸줉??鍮꾩뼱?덇굅??媛쒖닔媛 ?ㅻ쫭?덈떎.');

            testCase.verifyNotEmpty( ...
                loaded.Sessions, ...
                '濡쒕뱶???꾨줈?앺듃???몄뀡 紐⑸줉??鍮꾩뼱?덉뒿?덈떎.');
        ...
        function test_T3h_Mouse_MultiControllerDragIsolation(testCase)
            ...
            testCase.selectSessionViaPublicApi(app, sidA);
            markerLike = RouterTestController();
            splitterLike = RouterTestController();
        ...
        function test_T4_Event_Isolation(testCase)
            ...
            % the internal EventBus token/callback API.
             if ~isempty(which('verifyPhase4'))
                results = verifyPhase4();
        ...
        function test_T5_Explorer_Selection(testCase)
            ...
            testCase.assumeFalse( ...
                isempty(explorer), ...
                'ProjectExplorer 媛앹껜瑜?李얠쓣 ???놁뼱 ?뚯뒪?몃? 嫄대꼫?곷땲??');
            ...
            testCase.assumeTrue( ...
                ...
                'ProjectExplorer.selectSession/selectNode 硫붿꽌?쒓? ?놁뒿?덈떎.');
        ...
        function test_T6_Inspector_InvalidHandles(testCase)
             if ~isempty(which('verifyPhase6'))
                results = verifyPhase6();
                testCase.verifyDiagnosticHasNoFail( ...
                    results, ...
            ...
            testCase.assumeFalse( ...
                isempty(inspector), ...
                'Inspector 媛앹껜瑜?李얠쓣 ???놁뼱 ?뚯뒪?몃? 嫄대꼫?곷땲??');
            ...
            testCase.assumeFalse( ...
                ...
                'Inspector refresh/update public API媛 ?놁뒿?덈떎.');
        ...
        function test_T9_NonAscii_Path(testCase)
            ...
            nonAsciiDir = fullfile(testCase.TempDir, '?쒓?_寃쎈줈_?뚯뒪??);

            if ~exist(nonAsciiDir, 'dir')
                mkdir(nonAsciiDir);
            end

            project = flightdash.project.ProjectModel('?쒓? 寃쎈줈 ?꾨줈?앺듃');
            ...
            filePath = fullfile(nonAsciiDir, '?쒓?_?꾨줈?앺듃.frsproj');
        ...
        function test_TStress_MultiSession_Scrub(testCase)
            ...
            testCase.verifyTrue( ...
                closed, ...
                'S1 ?몄뀡 ??쓣 public API濡?醫낅즺?섎뒗 ???ㅽ뙣?덉뒿?덈떎.');
        ...
        function test_T11_VideoPlayer_MemoryCleanup_NoTimerLeak(testCase)
            % Phase 11 짠12: non-flaky memory diagnostic ??verify that
        ...
        function test_T11_SliderScrub_MarkerPreview_NoFullRedraw(testCase)
            ...
            % SliderLastRendered may stay NaN when no video is ready ??            % accept either NaN (video-not-ready early-exit) or the
        ...
        function test_T15_ControllerBase_AllTenInheritBase(testCase)
            % Phase 4 stabilization completion: all 10 controllers
            % inherit ControllerBase, so cleanup goes through a single
            % path and the verifyCallbackSafety leak probe applies
            % uniformly.
            classes = {'flightdash.controller.InfoController', ...
    ...
    methods (Access = private)
        ...
        function session = makeSessionWithDummyLinksInDir(testCase, displayName, baseDir)
            ...
            safeName = regexprep(char(displayName), '[^\w媛-??', '_');
