...
% This should normally be WARN until silent catches are cleaned up.
    ...
    for i = 1:numel(files)
        ...
        % Skip this diagnostic file family itself to avoid false positives.
        if endsWith(fpath, 'verifyRiskRegressionTests.m') || ...
                endsWith(fpath, 'verifyRiskRegressionT.m')
            continue;
        end
...
% This is a GUI smoke test and may open a uifigure.
    ...
    try
        ...
        status = 'SKIP';
        msg = sprintf('GUI creation unavailable: %s', ME.message);
        return;
    end
