classdef VideoPanel
    ...
    % - Col 6: I:AVI Video Player (5행 레이아웃)
    ...
    methods (Static)
        function ui = build(dataGrid, fIdx)
            ...
            ui.panelVideo.Layout.Column = 6;
            
            UIScale = flightdash.util.UIScale;   % High-DPI 행 높이 보정
            iGrid2 = uigridlayout(ui.panelVideo, [5 1]);
            ...
            uibutton(glVB, 'Text', 'AVI 파일 열기', 'FontSize', 11, ...
                'ButtonPushedFcn', @(~,~) EventBus.publish('AviFileRequested', AppEventData(fIdx)));
            ...
            ui.vidSyncBtn = uibutton(glSync, 'Text', '동기', ...
                ...
                'ButtonPushedFcn', @(~,~) EventBus.publish('VideoSyncRequested', AppEventData(fIdx)));
            ...
            ui.vidVdubSlider = uislider(vdubGrid, ...
                ...
                'ValueChangedFcn',  @(src,~) EventBus.publish('SliderChanged', AppEventData(fIdx, src)));
            ...
            uibutton(glNav, 'Text', '◄◄', 'FontSize', 11, 'FontWeight', 'bold', ...
                ...
                'ButtonPushedFcn', @(~,~) EventBus.publish('NavActionRequested', AppEventData(fIdx, 'first')));
            uibutton(glNav, 'Text', '◄', 'FontSize', 11, 'FontWeight', 'bold', ...
                'Tooltip', '이전 frame (-1)', ...
                'ButtonPushedFcn', @(~,~) EventBus.publish('NavActionRequested', AppEventData(fIdx, 'prev')));
            uibutton(glNav, 'Text', '►', 'FontSize', 11, 'FontWeight', 'bold', ...
                'Tooltip', '다음 frame (+1)', ...
                'ButtonPushedFcn', @(~,~) EventBus.publish('NavActionRequested', AppEventData(fIdx, 'next')));
            uibutton(glNav, 'Text', '►►', 'FontSize', 11, 'FontWeight', 'bold', ...
                'Tooltip', '10 프레임 앞으로 (+10)', ...
                'ButtonPushedFcn', @(~,~) EventBus.publish('NavActionRequested', AppEventData(fIdx, 'last')));
            ...
            uibutton(glHz, 'Text', '◄', 'FontSize', 10, ...
                'ButtonPushedFcn', @(~,~) EventBus.publish('HzAdjustRequested', AppEventData(fIdx, struct('target','video','delta',-1))));
            ui.vidVideoFpsInput = uispinner(glHz, 'Value', 15, 'Step', 1, ...
                'Limits', [1 1000], 'ValueDisplayFormat', '%d', 'FontSize', 10, ...
                'ValueChangedFcn', @(src,~) EventBus.publish('HzInputChanged', AppEventData(fIdx, struct('target','video','value',src.Value))));
            uibutton(glHz, 'Text', '►', 'FontSize', 10, ...
                'ButtonPushedFcn', @(~,~) EventBus.publish('HzAdjustRequested', AppEventData(fIdx, struct('target','video','delta',1))));
            uilabel(glHz, 'Text', '');
            ...
            uibutton(glHz, 'Text', '◄', 'FontSize', 10, ...
                'ButtonPushedFcn', @(~,~) EventBus.publish('HzAdjustRequested', AppEventData(fIdx, struct('target','data','delta',-1))));
            ui.vidDataFpsInput = uispinner(glHz, 'Value', 50, 'Step', 1, ...
                'Limits', [1 1000], 'ValueDisplayFormat', '%d', 'FontSize', 10, ...
                'ValueChangedFcn', @(src,~) EventBus.publish('HzInputChanged', AppEventData(fIdx, struct('target','data','value',src.Value))));
            uibutton(glHz, 'Text', '►', 'FontSize', 10, ...
                'ButtonPushedFcn', @(~,~) EventBus.publish('HzAdjustRequested', AppEventData(fIdx, struct('target','data','delta',1))));
            uilabel(glHz, 'Text', '');
            ...
            ui.vidCacheBudget = uidropdown(glHz, ...
                ...
                'ValueChangedFcn', @(src,~) EventBus.publish('CacheBudgetChanged', AppEventData(0, src.Value)));
        end
        ...
        function publishSliderChanging(fIdx, val)
            ...
            import flightdash.util.*
            if Throttle.instance().hit('LastSliderPublish', fIdx, AppConstants.SLIDER_THROTTLE_S)
                return;
            end
            EventBus.publish('SliderChanging', AppEventData(fIdx, val));
        end
    end
end
